...
public:
    ...
    std::vector<uint8_t> generateUserYUVFrame(const std::string& userId, uint64_t timestamp,
                                             uint32_t width, uint32_t height, const std::string& pattern = "user_specific") {
    ...
    MultiUserScenario createScenario(const std::string& scenarioName) {
        ...
        if (scenarioName == "small_meeting") {
            ...
            scenario.userIds = {"user1", "user2", "user3", "user4", "user5", "user6", "user7", "user8"};
            scenario.resolutions = {{640, 480}, {1280, 720}, {480, 640}, {320, 240}, 
                                  {640, 480}, {800, 600}, {1024, 768}, {720, 480}};
            scenario.patterns = {"user_specific", "user_specific", "user_specific", "user_specific",
                               "user_specific", "user_specific", "user_specific", "user_specific"};
...
protected:
    void SetUp() override {
        ...
        testDir_ = "./composition_test_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    ...
    for (uint32_t frameIdx = 0; frameIdx < scenario.durationFrames; frameIdx++) {
        ...
        for (size_t userIdx = 0; userIdx < scenario.userIds.size(); userIdx++) {
            auto yBuffer = framework_->generateUserYUVFrame(
                scenario.userIds[userIdx], timestamp,
                scenario.resolutions[userIdx].first, scenario.resolutions[userIdx].second,
                scenario.patterns[userIdx]
            );
            ...
            recordingSink.onVideoFrame(
                ...
                width, height, timestamp, scenario.userIds[userIdx]
            );
            ...
            for (int i = 0; i < 960; i++) {
                double time = static_cast<double>(timestamp) / 1000.0 + static_cast<double>(i) / 48000.0;
                int16_t sample = static_cast<int16_t>(sin(2.0 * M_PI * frequency * time) * 0.2 * 32767);
                audioSamples[i * 2] = sample;
                audioSamples[i * 2 + 1] = sample;
            }
            
            recordingSink.onAudioFrame(
                reinterpret_cast<uint8_t*>(audioSamples.data()),
                960, 48000, 2, timestamp, scenario.userIds[userIdx]
            );
