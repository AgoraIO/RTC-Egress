...
public:
    ...
    PreciseAVFrame generatePreciseFrame(const std::string& userId, uint64_t baseTimestamp,
                                       int64_t avOffsetMs = 0, uint32_t width = 640, uint32_t height = 480) {
        ...
        double frequency = 440.0 + (baseTimestamp % 1000) / 10.0; // Frequency changes with timestamp
        for (int i = 0; i < audioSamples; i++) {
            double time = static_cast<double>(baseTimestamp) / 1000.0 + static_cast<double>(i) / 48000.0;
    ...
    TimingMetrics analyzeAVSync(const std::vector<uint64_t>& videoTimestamps,
        ...
        for (size_t i = 0; i < minSize; i++) {
            double offset = static_cast<double>(audioTimestamps[i]) - static_cast<double>(videoTimestamps[i]);
            metrics.avOffsets.push_back(offset);
        }

        if (!metrics.avOffsets.empty()) {
            metrics.maxOffset = *std::max_element(metrics.avOffsets.begin(), metrics.avOffsets.end());
            metrics.minOffset = *std::min_element(metrics.avOffsets.begin(), metrics.avOffsets.end());
...
protected:
    void SetUp() override {
        ...
        testDir_ = "./av_sync_test_" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
...
TEST_F(AVSyncTest, PerfectAVSynchronization) {
    ...
    for (int i = 0; i < 60; i++) {
        ...
        auto frame = framework_->generatePreciseFrame("sync_perfect", timestamp, 0); // 0ms A/V offset

        recordingSink.onVideoFrame(
            frame.yBuffer.data(), frame.uBuffer.data(), frame.vBuffer.data(),
            frame.width, frame.width / 2, frame.width / 2,
            frame.width, frame.height, frame.videoTimestamp, frame.userId
        );

        recordingSink.onAudioFrame(
            frame.audioBuffer.data(), 960, 48000, 2,
            frame.audioTimestamp, frame.userId
        );
...
TEST_F(AVSyncTest, AudioLeadingVideo) {
    ...
    for (int i = 0; i < 60; i++) {
        ...
        auto frame = framework_->generatePreciseFrame("sync_audio_lead", timestamp, -40); // Audio 40ms ahead
...
TEST_F(AVSyncTest, VideoLeadingAudio) {
    ...
    for (int i = 0; i < 60; i++) {
        uint64_t timestamp = i * 33;
        auto frame = framework_->generatePreciseFrame("sync_video_lead", timestamp, 60); // Audio 60ms behind
