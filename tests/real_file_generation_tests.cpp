   ...
   public:
    ...
    std::vector<uint8_t> generateAudioFrame(const std::string& userId, uint64_t timestamp,
        ...
        for (int i = 0; i < samples; i++) {
            double time =
                static_cast<double>(timestamp) / 1000.0 + static_cast<double>(i) / sampleRate;
...
class RealFileGenerationTest : public ::testing::Test {
   protected:
    void SetUp() override {
        ...
        testDir_ = "./real_files_test_output_" +
...
TEST_F(RealFileGenerationTest, RealJPEGSnapshotGeneration) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        auto videoFrame =
            generator_->generateVideoFrame("jpeg_user", timestamp, 1280, 720, pattern);
        ...
        snapshotSink.onVideoFrame(yBuffer.data(), uBuffer.data(), vBuffer.data(), 1280, 640,
                                  640,  // strides
                                  1280, 720, timestamp, "jpeg_user");
...
TEST_F(RealFileGenerationTest, RealMP4RecordingWithAVSync) {
    ...
    std::thread videoThread([&]() {
        for (int i = 0; i < durationSeconds * 30 && recording; i++) {
            ...
            recordingSink.onVideoFrame(yBuffer.data(), uBuffer.data(), vBuffer.data(), 1280, 640,
                                       640,  // strides
    ...
    std::thread audioThread([&]() {
        for (int i = 0; i < durationSeconds * 50 && recording; i++) {
            ...
            auto audioBuffer =
                generator_->generateAudioFrame("mp4_user", timestamp, 960, 48000, 2, 440.0f);

            recordingSink.onAudioFrame(audioBuffer.data(), 960, 48000, 2, timestamp, "mp4_user");
            audioFramesSent++;
    ...
    for (size_t userIdx = 0; userIdx < users.size(); userIdx++) {
        userThreads.emplace_back([&, userIdx]() {
            for (int i = 0; i < frameCount; i++) {
                ...
                recordingSink.onVideoFrame(yBuffer.data(), uBuffer.data(), vBuffer.data(), 640, 320,
                                           320,  // strides
...
TEST_F(RealFileGenerationTest, RealAVSynchronizationTest) {
    ...
    for (int i = 0; i < durationFrames; i++) {
        ...
        auto videoFrame =
            generator_->generateVideoFrame("sync_user", baseTimestamp, 640, 480, "moving_bar");
        ...
        for (int a = 0; a < 2; a++) {
            ...
            auto audioBuffer =
                generator_->generateAudioFrame("sync_user", audioTimestamp, 480, 48000, 2);

            recordingSink.onAudioFrame(audioBuffer.data(), 480, 48000, 2, audioTimestamp,
                                       "sync_user");
        }
