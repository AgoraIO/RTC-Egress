...
### C++ Components
- **Build all components**: `./build.sh all [AGORA_SDK_URL]` - Downloads Agora SDK, installs dependencies, and builds both C++ and Go components
...
- `make build` - Creates build directory and compiles C++ components
- `make clean` - Removes build and out directories
...
- `POST /api/v1/egress/start` - Start recording session
- `POST /api/v1/egress/stop/{egress_id}` - Stop recording
- `GET /api/v1/egress/status/{egress_id}` - Get session status
...
- Missing Agora SDK: Run `./build.sh all` with a valid SDK download URL
