...
// OpenCV includes for frame processing
...
// Helper function to save YUV frame as JPEG
static void SaveYuvToJpeg(const agora::media::base::VideoFrame& yuvFrame,
                          const std::string& filename) {
    try {
        // Extract directory path
        std::string dir_str = filename.substr(0, filename.find_last_of("/\\"));
        if (!dir_str.empty() && !CreateDirectoryIfNotExists(dir_str)) {
            AG_LOG(ERROR, "Failed to create directory %s", dir_str.c_str());
            return;
        }

        // Convert YUV to BGR (OpenCV uses BGR by default)
        cv::Mat yuvMat(yuvFrame.height * 3 / 2, yuvFrame.width, CV_8UC1);
        // Copy YUV data to the matrix
        memcpy(yuvMat.data, yuvFrame.yBuffer, yuvFrame.height * yuvFrame.width * 3 / 2);

        cv::Mat bgrMat;

        try {
            if (yuvFrame.type == agora::media::base::VIDEO_PIXEL_I420) {
                cv::cvtColor(yuvMat, bgrMat, cv::COLOR_YUV2BGR_I420);
            } else if (yuvFrame.type == agora::media::base::VIDEO_PIXEL_NV12) {
                cv::cvtColor(yuvMat, bgrMat, cv::COLOR_YUV2BGR_NV12);
            } else {
                AG_LOG(ERROR, "Unsupported YUV format: %d", yuvFrame.type);
                return;
            }
        } catch (const cv::Exception& e) {
            AG_LOG(ERROR, "OpenCV exception in YUV conversion: %s", e.what());
            return;
        }
        ...
        compression_params.push_back(95);  // 95% quality

        try {
            if (!cv::imwrite(filename, bgrMat, compression_params)) {
                AG_LOG(ERROR, "Failed to save frame to %s", filename.c_str());
            } else {
                AG_LOG(INFO, "Saved frame to %s", filename.c_str());
            }
        } catch (const cv::Exception& e) {
            AG_LOG(ERROR, "OpenCV exception saving image: %s", e.what());
        }
    } catch (const std::exception& e) {
        AG_LOG(ERROR, "Exception in SaveYuvToJpeg: %s", e.what());
    }
}

namespace rtc {

RtcClient::RtcClient() : saveFrames_(false), lastSaveTime_(std::chrono::steady_clock::now()) {
    // Set up the default frame callback to handle periodic saving
    frameCallback_ = [this](const agora::media::base::VideoFrame& videoFrame,
                            const std::string& userId) {
        if (!saveFrames_) return;

        auto now = std::chrono::steady_clock::now();
        auto elapsed =
            std::chrono::duration_cast<std::chrono::seconds>(now - lastSaveTime_).count();

        if (elapsed >= saveInterval_) {
            // Generate timestamped filename
            auto now = std::chrono::steady_clock::now();
            auto in_time_t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            std::stringstream ss;
            ss << saveDirectory_ << "/frame_"
               << std::put_time(std::localtime(&in_time_t), "%Y%m%d_%H%M%S") << "_" << userId
               << ".jpg";

            // Save the frame
            SaveYuvToJpeg(videoFrame, ss.str());
            lastSaveTime_ = now;
        }
    };
}
